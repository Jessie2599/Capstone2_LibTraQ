...
def open_add_record_window():
    ...
    def generate_pdf(library_id_no_get, fname_get, mname_get, lname_get, sex_get, course_get, status_get, photo_data,
        ...
        # Create a PDF document with one page
        c = canvas.Canvas(str(file_path), pagesize=(letter[0], letter[1]))

        for card_num in range(2):
            card_width = letter[0] / 2 - 20
            card_height = letter[1] / 2 - 20

            # Draw a rectangular border to resemble an ID card
            border_x = 10 + (card_width + 20) * (card_num % 2)  # Adjust x-coordinate based on card number
            c.rect(border_x, 10, card_width, card_height)

            if card_num == 0:
                header_image_path = "images/header.png"
                header_image = Image.open(header_image_path)
                header_image_width, header_image_height = header_image.size
                c.drawInlineImage(header_image, 16 + border_x, card_height - 50, width=header_image_width / 4,
                                  height=header_image_height / 3)

                # Title for the ID card
                c.setFont("Helvetica-Bold", 20)
                c.drawString(80 + border_x, card_height - 70, "Library Card")
            ...
            if card_num == 0:
                ...
                c.setLineWidth(1)
                c.drawImage(ImageReader(photo), info_x, card_height - 190, width=photo_width / 2,
                            height=photo_height / 2)
                c.rect(info_x, card_height - 190, photo_width / 2, photo_height / 2)
                ...
                c.setLineWidth(1)
                c.drawImage(ImageReader(qr_image), info_x + 130, card_height - 190, width=qr_width / 2,
                            height=qr_height / 2)
                c.rect(info_x + 130, card_height - 190, qr_width / 2, qr_height / 2)
            else:
                # Add a placeholder image on the second card
                placeholder_image_path = "images/card.png"  # Provide the path to your placeholder image
                placeholder_image = Image.open(placeholder_image_path)
                placeholder_width, placeholder_height = placeholder_image.size
                c.drawImage(ImageReader(placeholder_image), info_x, card_height - 350,
                            width=placeholder_width / 2.2, height=placeholder_height / 1.3)
